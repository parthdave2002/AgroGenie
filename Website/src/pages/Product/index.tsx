                      ...
                      <Swiper modules={[Navigation, Autoplay]} spaceBetween={16} slidesPerView={1} loop={true} autoplay={{ delay: 3000, disableOnInteraction: false, }} >
                        {product?.product_pics.map((img: any, index: number) => (
                          <SwiperSlide key={index}>
                            <LazyLoadImage effect="blur" src={img} alt={`Product image ${index + 1}`} className="mx-auto max-h-[210px] w-[12rem] h-[12rem] object-contain cursor-pointer" onClick={() => DetailspageCall(product?._id)} />
                          </SwiperSlide>
                        ))}
                      </Swiper>
                    </figure>
