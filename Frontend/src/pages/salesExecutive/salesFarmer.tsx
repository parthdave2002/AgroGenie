...
import { ProfilePropsData, Referdata } from "../../types/types";
...
const SalesFarmerDetailsPage : FC  <ProfilePropsData> = function ({ setDatactive, openProfile,setOpenProfile })  {
    ...
    const [Referrerdata, setReferrerdata] = useState<Referdata | undefined>()
    ...
    useEffect(() =>{
        if(CheckCustomerExistlist){
            const referrer = CheckCustomerExistlist?.data?.referrer;
            setReferrerdata(referrer && referrer)
        } 
    ...
    return (
      <>
        {openProfile == true ? 
                <SalesFarmerDashboard Mobile_number={Mobile_number} Referrerdata={Referrerdata}  setOpenProfile={CloseProfileCall} />
         : 
            <>
                <div className="flex justify-between">
                    <div className="flex flex-col self-center">
                        ...
                        <div className="text-[2rem] font-semibold text-DarkBackground dark:text-TitaniumWhite">  Callbacks ({String(Farmerdata?.length).padStart(2, '0')}) </div>
                    </div>
                    <SalesMobileInput datatype='number' className="py-2 px-6 border-0  rounded-full text-[2rem] text-SharkGray font-bold relative shadow-xl dark:shadow-xl  shadow-inner shadow-indigo-200  dark:shadow-SharkGray/50 dark:bg-TranquilBlack dark:text-TitaniumWhite" placeholder="Enter mobile number" value={Mobile_number} handleChange={(data) =>handleChange(data)} handleClickCall={handleClickCall}  />
                </div>

                <div className="grid  xl:grid-cols-4 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Farmerdata &&   Farmerdata.map((item: any, k: number) => (
                    <div className="bg-White dark:bg-DarkBackground p-2 rounded-[3rem] my-2" key={k}>
                        <div className= { `${  k % 2 === 0 ? "bg-[#fdf6e4]" : "bg-[#f4f9fd]" } dark:bg-Cosmos px-[0.8rem] py-[1rem] rounded-t-[3rem] flex flex-col gap-y-1 relative`}>
                            {k % 2 === 0 && (
                                <>
                        ...
                        <div className="flex justify-between mt-3">
                            <div className="text-TranquilBlack dark:text-TitaniumWhite text-[1rem] truncate flex-1">   {item?.customer?.firstname}  {item?.customer?.middlename}  {item?.customer?.lastname}  </div>
                            <div className="text-TranquilBlack dark:text-TitaniumWhite  text-[1rem] text-end flex-1 ">   {item?.customer?.mobile_number}   </div>
                        </div>
                        <div className="flex justify-between mt-3">
                            <div className="text-TranquilBlack dark:text-TitaniumWhite text-[1rem] truncate flex-1">   Callback Date </div>
                            <div className="text-TranquilBlack dark:text-TitaniumWhite  text-[1rem] text-end flex-1 ">   {moment(item?.future_order_date).format("DD-MM-YYYY")}   </div>
                        </div>
                        <div className="text-SharkGray mt-3 text-[1.2rem] text-center border border-SharkGray rounded-md size-fit px-2 cursor-pointer self-center px-[4rem]" onClick={() => CallBackCall(item?.customer?.mobile_number) }>   Call    </div>
                        </div>

                        <div className="py-[0.5rem] rounded-xl flex justify-around">
                        <div className="text-Hydrocarbon dark:text-TitaniumWhite  text-center  flex flex-col">
                                <span className="text-[1rem]"> Complain </span>
                                <span className="text-[1.4rem] text-DarkBackground dark:text-TitaniumWhite font-bold"> {item?.totalComplain}   </span>
                            </div>
                            <div className="text-Hydrocarbon dark:text-TitaniumWhite  text-center  flex flex-col">
                                <span className="text-[1rem]"> Order </span>
                                <span className="text-[1.4rem] text-DarkBackground dark:text-TitaniumWhite font-bold"> {item?.totalConfirmOrders}   </span>
                            </div>
                            <div className="text-Hydrocarbon dark:text-TitaniumWhite  text-center  flex flex-col">
                                <span className="text-[1rem]"> Return </span>
                                <span className="text-[1.4rem] text-DarkBackground dark:text-TitaniumWhite font-bold"> {item?.totalReturnOrder}   </span>
                            </div>
                        </div>
                    </div>
                    ))}
