        ...
        <>
          <div className='flex justify-between'>
            <div className='flex flex-col self-center'>
              ...
              <div className="text-[2rem] font-semibold text-DarkBackground dark:text-TitaniumWhite">Future Orders  </div>
            </div>
            <SalesMobileInput placeholder="Search Order" className="py-2 px-6 border-0  rounded-full text-[2rem] text-SharkGray font-bold relative shadow-xl dark:shadow-xl  shadow-inner shadow-indigo-200  dark:shadow-SharkGray/50 dark:bg-TranquilBlack dark:text-TitaniumWhite" value={orderId} handleClickCall={handleClickCall} handleChange={(data) =>handleChange(data)} />
          </div>
                    ...
                    <div className="bg-White dark:bg-DarkBackground p-2 rounded-[3rem] my-2" key={k}>
                        <div className={`${k % 2 === 0 ? "bg-[#fdf6e4]" : "bg-[#f4f9fd]"} dark:bg-Cosmos px-[0.8rem] py-[1rem] rounded-t-[3rem] flex flex-col gap-y-1 relative`}>
                          ...
                          <div className="flex justify-between mt-2">
                            <div className="text-TranquilBlack dark:text-TitaniumWhite  text-[1rem]  flex-1 ">  Created Date    </div>
                            <div className="text-TranquilBlack dark:text-TitaniumWhite text-[1rem] truncate flex-1 text-end">   {moment(item?.order_date).format("DD-MM-YYYY")}  </div>
                          </div>

                          <div className="flex justify-between mt-2">
                            <div className="text-TranquilBlack dark:text-TitaniumWhite text-[1rem] truncate flex-1">  Followup Date  </div>
                          ...
                          <div className="text-SharkGray mt-3 text-[1.2rem] text-center border border-SharkGray rounded-md size-fit px-2 cursor-pointer self-center px-[4rem]" onClick={() => CallBackCall(item?.order_id)}>   Call    </div>
                        </div>
