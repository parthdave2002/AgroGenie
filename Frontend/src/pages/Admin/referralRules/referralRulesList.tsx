...
import { getReferralRulelist, DeleteReferralRulelist, ChangeStatusReferralRulelist } from "../../../Store/actions";
...
const ReferralRuleListPage: FC = function () {
  ...
  //------------ Access Data Code start------------
    const { ReferralRulelist,  ReferralRulelistSize, TotalReferralRuleData, CurrentPage, permissionsdata } = useSelector((state: any) => ({
      ReferralRulelist: state.Referral.ReferralRulelist,
      ReferralRulelistSize: state.Referral.ReferralRulelistSize,
      TotalReferralRuleData: state.Referral.TotalReferralRuleData,
      CurrentPage: state.Referral.CurrentPage,
      permissionsdata: state.Login.permissionsdata
    }));

    const accessList = UseAccessList(permissionsdata, "Referral Rules");
  ...
  // ------------- Get  Data From Reducer Code Start --------------
    useEffect(() => {
      ...
      dispatch(getReferralRulelist(requserdata));
    }, [dispatch, PageNo, RoePerPage, searchData]);

    useEffect(() => {  
      setReferralRulelistdata(ReferralRulelist ? ReferralRulelist : null);
      setTotalListData(TotalReferralRuleData ? TotalReferralRuleData : 0);
      setCurrentPageNo(CurrentPage ? CurrentPage : 1);
    }, [ReferralRulelist,  ReferralRulelistSize, TotalReferralRuleData, CurrentPage]);
  ...
  // ------------  Delete Code Start ------------
    ...
    const DeleteCrop = () => {
      ...
      dispatch(DeleteReferralRulelist(rqeuserdata));
      setisOpenDelteModel(false);
    };
  // -------  Delete Code End ---------------

  const OpenAddModel = () =>{
    navigate("/referral-rules/add")
  }

  const ChangestatusFuncall = (id:any) =>{
      let rqeuserdata = { id: id };
      dispatch(ChangeStatusReferralRulelist(rqeuserdata))
  }

  let Name = "Referral Rules List";
  let Searchplaceholder = "Search For Referral Rules";
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="flex items-center gap-x-3">
          ...
          {accessList?.delete ? <Button gradientDuoTone="purpleToPink" onClick={() => DeleteFuncall(row?._id)}><div className="flex items-center gap-x-2 deletebutton"> <HiTrash className="text-lg" />  Delete Referral Rules</div> </Button> : null}
        </div>
      ),
    },
    ...
    <>
      <NavbarSidebarLayout isSidebar={true} isNavbar={true} >
        ...
        <div className="flex items-center justify-center my-3">
          <p className="text-[1.3rem] dark:text-gray-50"> 1 Point = ₹0.50 </p>
        </div>
...
export default ReferralRuleListPage;