...
const ProductAddPage: FC = function () {
    ...
    const handleChange = (id: string | number, field: keyof DescriptionData, newValue: string) => {
        setInputs((prev) =>
    ...
    // ------ isBestSelling code start ------
    const [selectedBestSellingOption, setSelectedBestSellingOption] = useState(null);
    const [selectedBestSellingid, setSelectedBestSellingid] = useState<null | boolean>(null);
    const [validateBestSelling, setValidateBestSelling] = useState(0);

    const IsBestSellingdata = (data: any) => {
        if (!data) {
            setSelectedBestSellingid(null);
            setSelectedBestSellingOption(null);
            setValidateBestSelling(1)
        } else {
            setSelectedBestSellingid(data.value);
            setSelectedBestSellingOption(data);
            setValidateBestSelling(0)
        }
    };
    // ------ isBestSelling code end ------

// ------ ismost Popular code start ------
    const [selectedMostPopularOption, setSelectedMostPopularOption] = useState(null);
    const [selectedMostPopularid, setSelectedMostPopularid] = useState<null | boolean>(null);
    const [validateMostpopular, setValidateMostpopular] = useState(0);

    const IsMostpopulardata = (data: any) => {
        if (!data) {
            setSelectedMostPopularid(null);
            setSelectedMostPopularOption(null);
            setValidateMostpopular(1)
        } else {
            setSelectedMostPopularid(data.value);
            setSelectedMostPopularOption(data);
            setValidateMostpopular(0)
        }
    };
...
// ------ show on web code start ------
    ...
    const IsShowwebdata = (data: any) => {
        if (!data) {
            setSelectedShowwebid(null);
            setSelectedShowwebOption(null);
            setValidateShowweb(1)
        } else {
            setSelectedShowwebid(data.value);
            setSelectedShowwebOption(data);
            setValidateShowweb(0)
        }
    };
    ...
    const validation = useFormik({
        ...
        onSubmit: (values) => {
            ...
            formData.append("hsn_code", values?.hsn_code);
             formData.append("isBestSelling", JSON.stringify(selectedBestSellingid));
            formData.append("isMostpopular", JSON.stringify(selectedMostPopularid));
            formData.append("isShowonsite",JSON.stringify(selectedShowwebid));
