  ...
  const Closecall = () =>{
    ...
    const calculateOrderSummary = () => {
      ...
      openDetailIData?.products?.forEach((item: any) => {
        ...
        totalGrandTotal = Math.max(0, totalBeforeCoupon - (openDetailIData?.coupon?.amount ?? 0) - ((openDetailIData?.coupon?.amount ?? 0)));
      });

      return { totalSubtotal, totalDiscount, totalGST, grandTotal: totalSubtotal + totalGST, totalGrandTotal };
    };

    const { totalSubtotal, totalDiscount, totalGST } = calculateOrderSummary();
    ...
    <>
      <div className='flex justify-between'>
        <div className="flex gap-x-5"> 
          <span className='text-[2rem] font-semibold text-DarkBackground dark:text-TitaniumWhite'> Order ID :  {orderId}  </span> 
      ...
      <div className="flex flex-col md:flex-row min-w-full justify-between gap-4">
        ...
        <div className="md:flex-1">
          <div className="border dark:border-Hydrocarbon dark:bg-Cosmos my-4 p-3 rounded-xl w-full">
            <div className="text-[1.2rem] font-semibold text-DarkBackground dark:text-TitaniumWhite"> Order Item  </div>
              ...
              {openDetailIData?.products  && openDetailIData?.products.map((data:any , k:number) =>(
                <div key={k} className='flex justify-between gap-x-4 my-4'>
                  <div className='flex gap-x-3'>
                    ...
                    <div>
                      <div className='truncate max-w-[25rem] text-Hydrocarbon dark:text-SoothingBlueGrey'> {data?.id?.name?.englishname} </div>
                      <div className='truncate max-w-[25rem] text-Hydrocarbon text-[0.8rem] dark:text-SoothingBlueGrey'> {data?.id?.tech_name?.english_tech_name} </div>
                    </div>
                  </div>

                  <div className='flex gap-x-3'> 
                    <div className='py-1 px-4 border rounded-xl self-center dark:text-SoothingBlueGrey'> {data?.quantity}  X  {data?.id?.price} </div>
                    <div className='text-center self-center dark:text-SoothingBlueGrey'> - {data?.id?.discount} </div>
                    <div className='text-center self-center dark:text-SoothingBlueGrey'> = {Math.round(data?.quantity * data?.id?.price - data?.id?.discount)}</div>
                  </div>
                  
                  { openDetailIData?.status == "confirm" ?  <div className='text-center self-center  bg-indigo-600 hover:bg-indigo-700 text-TitaniumWhite rounded-lg cursor-pointer flex gap-x-2 px-3 py-1.5' onClick={() => CompainCall(data?.id?._id)}> <FaExclamationTriangle className='self-center text-xl'  /> Complain </div> : null }
                </div>
              ))}
            </div>
          </div>

          <div className="border dark:border-Hydrocarbon dark:bg-Cosmos my-4 p-4 rounded-xl w-full">
            <div className="text-2xl font-semibold text-DarkBackground dark:text-TitaniumWhite mb-8"> Order Summary </div>

            {/* <div className="flex flex-col items-end space-y-3">
              <div className="text-lg font-semibold text-SharkGray dark:text-SoothingBlueGrey flex justify-between w-full max-w-xs"> <span>Total Discount </span> <span >: {totalDiscount.toFixed(2)} Rs.</span> </div>
              <div className="text-lg font-semibold text-SharkGray dark:text-SoothingBlueGrey flex justify-between w-full max-w-xs"> <span>Total Subtotal </span> <span > : {totalSubtotal.toFixed(2)} Rs.</span></div>
              <div className="text-lg font-semibold text-SharkGray dark:text-SoothingBlueGrey flex justify-between w-full max-w-xs"> <span>Total GST </span> <span > : {totalGST.toFixed(2)} Rs.</span> </div>
              {openDetailIData?.coupon?.amount ? 
                <div className="text-lg font-semibold text-SharkGray dark:text-SoothingBlueGrey flex justify-between w-full max-w-xs"> <span> Coupon </span> <span > : - {openDetailIData?.coupon?.amount.toFixed(2)} Rs.</span> </div>
              : null}
            </div> */}

            <div className="flex flex-col justify-self-end w-full max-w-md  dark:bg-Cosmos  space-y-4">
                <div className="flex justify-between w-full text-base md:text-lg font-medium text-Hydrocarbon dark:text-SoothingBlueGrey">
                  <span>Total Discount</span>
                  <span className="font-semibold text-Cosmos dark:text-White">{Math.round(totalDiscount)} Rs.</span>
                </div>
                
                <div className="flex justify-between w-full text-base md:text-lg font-medium text-Hydrocarbon dark:text-SoothingBlueGrey">
                  <span>Total Subtotal</span>
                  <span className="font-semibold text-Cosmos dark:text-White">{Math.round(totalSubtotal)} Rs.</span>
                </div>
                
                <div className="flex justify-between w-full text-base md:text-lg font-medium text-Hydrocarbon dark:text-SoothingBlueGrey">
                  <span>Total GST</span>
                  <span className="font-semibold text-Cosmos dark:text-White">{Math.round(totalGST)} Rs.</span>
                </div>
                ...
                {openDetailIData?.wallet_poins && (
                  <div className="flex justify-between w-full text-base md:text-lg font-medium text-green-600 dark:text-green-400">
                    <span>Wallet Points</span>
                    <span className="font-semibold">- {Math.round(openDetailIData?.wallet_poins) ?? 0} Rs.</span>
                  </div>
                )}
              </div>

            <div className="flex justify-end items-center text-xl font-semibold text-SharkGray dark:text-SoothingBlueGrey mt-4 gap-x-4 "> <span className='text-[1.5rem]'>Grand Total</span> <span className='min-w-[11rem] text-end'> : {openDetailIData?.total_amount} Rs.</span> </div>
          </div>

          { openDetailIData?.status == "confirm" ?
          <div className='flex justify-end gap-x-4'>
            {moment(openDetailIData?.added_at).format("DD-MM-YYYY") === Todaydate ?
              // <div className='text-center self-center  bg-red-600 border border-red-600 hover:bg-red-600 hover:border-red-500 rounded-lg cursor-pointer flex gap-x-2 px-4 py-2 text-TitaniumWhite'  onClick={() => ReturnComplainCall()}  > <FaTruck className='self-center h-6 w-6' /> Cancel </div>
              ...
              :
              <div className='text-center self-center  bg-red-600 border border-red-600 hover:bg-red-600 hover:border-red-500 rounded-lg cursor-pointer flex gap-x-2 px-4 py-2 text-TitaniumWhite'  onClick={() => ReturnComplainCall("return")}  > <FaTruck className='self-center h-6 w-6' /> Return </div>
            }
            <div className='text-center self-center  bg-indigo-600 hover:bg-indigo-700 text-TitaniumWhite rounded-lg cursor-pointer  flex gap-x-2 px-4 py-2' onClick={() => CreateComplainCall()}> <FaExclamationTriangle className='self-center '  /> Complain </div>
          </div>
          : null }
        </div>
        ...
        <div className="md:w-[20rem] w-full flex flex-col gap-4 mt-5">
           {openDetailIData?.status === "confirm" ?
          <div className="border dark:border-Hydrocarbon dark:bg-Cosmos p-3 rounded-xl w-full flex flex-col gap-y-3 cursor-pointer" onClick={ () =>OpenInviceModal()}>
            <div className='dark:text-SoothingBlueGrey text-[1.2rem] font-semibold'>Invoice</div>  
          </div>
          : null }

          <div className="border dark:border-Hydrocarbon dark:bg-Cosmos p-3 rounded-xl w-full flex flex-col gap-y-3">
            <div className='dark:text-SoothingBlueGrey text-[1.2rem] font-semibold'>Customer</div>
            <div className='dark:text-SoothingBlueGrey flex gap-x-3 truncate max-w-[18rem]'><FaUserAlt className='self-center h-7-w-7'  />  <span> {data?.firstname} {data?.middlename} {data?.lastname} </span> </div>
          </div>

          <div className="border dark:border-Hydrocarbon dark:bg-Cosmos p-3 rounded-xl w-full flex flex-col gap-y-3">
            <div className='dark:text-SoothingBlueGrey text-[1.2rem] font-semibold'>Contact Information</div>
            <div className='dark:text-SoothingBlueGrey flex gap-x-3'><FaPhoneVolume className='self-center h-7-w-7' />  <span> {data?.mobile_number} , {data?.alternate_number} </span> </div>
          </div>

          <div className="border dark:border-Hydrocarbon dark:bg-Cosmos p-3 rounded-xl w-full flex flex-col gap-y-3">
            <div className='dark:text-SoothingBlueGrey text-[1.2rem] font-semibold'>Shipping Address</div>
            <div className='dark:text-SoothingBlueGrey '>  {data?.customerAddress } </div>
            <div className='dark:text-SoothingBlueGrey'> District :   {data?.district_name } </div>
            <div className='dark:text-SoothingBlueGrey'> Taluka :  {data?.taluka_name } </div>
            <div className='dark:text-SoothingBlueGrey'> Village :  {data?.village_name } </div>
            <div className='dark:text-SoothingBlueGrey'> Pincode : {data?.pincode } </div>
          </div>
        </div>
      </div>
                            ...
                            >
                              ...
                              <div className="flex flex-col">
                                <div className="text-center mb-2">
                                  <h6 className="text-[0.9rem]  text-SharkGray ">  એગ્રી ભારત કંપનીમાંથી 100% ખાતરીબંધ બ્રાન્ડેડ કૃષિ પ્રોડક્ટ ઘરે બેઠા ઓર્ડર કરવા અને ખેતીને લગતી વધુ માહિતી માટે  </h6>
                                  <h6 className="text-[0.9rem]  text-SharkGray ">  હેલ્પલાઇન નંબર 9100029429/9100029329 પર સંપર્ક કરો!!!  જય કિસાન  </h6>
                                </div>
            
                                <div className="flex justify-between items-center mb-4">
                                  <img src={logo} alt="Company Logo" className="w-24" />
                                  <h1 className="text-3xl font-bold text-Cosmos text-right">INVOICE</h1>
                                </div>
            
                                <div className="flex justify-between items-center mb-4 border-b pb-4">
                                  <div>
                                    <p className="text-SharkGray text-[0.9rem]"> Warehouse-1, Diu Road, </p>
                                    <p className="text-SharkGray text-[0.9rem]">At: Kesariya - 362560, Ta.: Una,</p>
                                    <p className="text-SharkGray text-[0.9rem]">  Dist.: Gir Somnath, Gujarat </p>
                                    <p className="text-SharkGray text-[0.9rem]">  GST :  </p>
                                  </div>
                                  <div className="text-right">
                                    <p className="text-SharkGray text-sm"><strong> Pesticide Lic No: </strong> GRS/FP1230000664/2023-24 </p>
                                    <p className="text-SharkGray text-sm"><strong> Seeds Lic No: </strong>  GRS/FSR230000774/2023-24	 </p>
                                    <p className="text-SharkGray text-sm"> <strong> Ferlitizer Lic No: </strong> GRS/FFR230000775/2023-24 </p>
                                    <p className="text-SharkGray text-sm"> E-mail: agribharat2023@gmail.com</p>
                                    <p className="text-SharkGray text-sm"> Contact : 91000 29329/91000 29429 </p>
                                  </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 mb-4">
                                  <div>
                                    ...
                                    <p className="text-Hydrocarbon text-[0.9rem]">  {UserDataList?.customer?.address}   </p>
                                    <p className="text-Hydrocarbon text-[0.9rem]">  {UserDataList?.customer?.village_name}, {UserDataList?.customer?.taluka_name},  {UserDataList?.customer?.district_name},  </p>
                                    <p className="text-Hydrocarbon text-[0.9rem]">  {UserDataList?.customer?.state?.name} -  {UserDataList?.customer?.pincode}  </p>
                                    <p className="text-Hydrocarbon text-[0.9rem]">  Contact : {UserDataList?.customer?.mobile_number}, {UserDataList?.customer?.alternate_number} </p>
                                  </div>
                                  <div className="text-right">
                              ...
                              <div className="flex-1 overflow-hidden">
                                <table className="w-full border border-SoothingBlueGrey">
                                  <thead className="bg-TranquilBlack text-White">
                                    <tr>
                                  ...
                                  <tbody>
                                    {productChunk.map((item, k) => {
                                      ...
                                      return (
                                        <tr key={k} className="bg-White text-center text-[0.8rem]">
                              ...
                              {pageIndex === productChunks.length - 1 && (
                                <>
                                  <div className="mt-4 flex">
                                    ...
                                    <div className="flex-1 text-[1rem]">
                                      <div className="space-y-1">
                                        ...
                                        {/* Coupon Section */}
                                        {UserDataList?.coupon && (
                                          <div className="border-t border-dashed border-SilverSteel py-2 space-y-1">
                                            <div className="flex justify-between">
                                              <div className="flex">
                                  ...
                                  <div className="text-2xl font-bold bg-TranquilBlack text-White px-3 py-2  text-right  leading-tight antialiased">Total : ₹{Math.round(total) ?? 0}</div>

                                  <div className="mt-4 text-[0.8rem] text-TranquilBlack">
                                      <div className="flex justify-between"> 
                                          <div className="text-[1.2rem]" ><strong>Terms & Conditions:</strong>  </div>
                                          <img src="/images/authentication/signature.webp" className="mb-3 border-b border-dashed border-SilverSteel pb-1 w-[8rem] h-[3rem]" /> 
                                      </div>
                                  ...
                                  <div className="mt-4 text-center text-Hydrocarbon">
