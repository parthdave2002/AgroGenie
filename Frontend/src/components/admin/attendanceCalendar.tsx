...
export default function AttendanceCalendar({ users }: AttendanceCalendarProps) {
    ...
    return (
        <div className="p-4 bg-White dark:bg-Cosmos rounded-lg shadow">
            {/* Header */}
                            ...
                            <div className="overflow-x-auto">
                                ...
                                <div className="grid grid-cols-[200px_repeat(31,minmax(28px,28px))] gap-x-3 text-sm mb-2">
                                    ...
                                    {monthDays.map((day, idx) => (
                                        <div key={idx} className="text-center text-SharkGray">  {day.format("dd")}  </div>
                                    ))}
                                </div>
                                ...
                                {users.map((user: any, index: number) => (
                                    <div key={index} className="grid grid-cols-[200px_repeat(31,minmax(28px,28px))] gap-x-3  py-2 items-center">
                                        <div className="flex items-center gap-2 font-medium text-DarkBackground dark:text-White">{user.name} </div>
                                        ...
                                        {monthDays && monthDays.map((day: any, idx: number) => {
                                            ...
                                            const cellClass = `w-7 h-7 flex items-center justify-center text-sm rounded-full border border-SoothingBlueGrey dark:text-White
